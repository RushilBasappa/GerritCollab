...
def pull_and_checkout(cl):
    ...
    COMMAND = "git fetch \"https://{USERNAME}@gerrit.p.sojern.net/a/sojern/code\" refs/changes/{XX}/{YYYY}/{PP} && git checkout FETCH_HEAD && git checkout -b {YYYY}_{PP}".format(
        XX=XX, YYYY=YYYY, PP=PP, USERNAME=USERNAME)
    print(COMMAND)
